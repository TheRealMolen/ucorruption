...
;    char* const pbuf = 0x2402
;    getsn( pbuf, 0xff )  - at most 255 bytes from console to 2402
...
; const char* pc = pbuf - 1    <-- pc in r15
; while( *(++pc) ) {}
...
; u16 unamelen = pc - pbuf    <-- unamelen in r11
; unamelen in range [0,0xff]
459e:  0b4f           mov	r15, r11
45a0:  3b80 0224      sub	#0x2402, r11

; const u16 localpasslen = 31
; char localpass[localpasslen+1]
; u16 somebollocks[1]
; strcpy( localpass, pbuf )
45a4:  3e40 0224      mov	#0x2402, r14
45a8:  0f41           mov	sp, r15
45aa:  b012 f446      call	#0x46f4 <strcpy>

; if( (r11 & 0xff) >= 33 ) {      !! overrun - localpass only 32 chars long
;                        !! also: only byte compare, so 0x120 would pass
...
; uint maxpwdlen = 31 - unamelen
;           <----  if unamelen == 32 then we end up with 0xffff 
...
; maxpwdlen &= 0x1ff
;            <-- ok so max is 0x1ff = 511
45d2:  3f40 0224      mov	#0x2402, r15


; getsn( pbuf, maxpwdlen )
...
; strcpy( localpass + unamelen, pbuf )
...
; uint16 uinputlen = unamelen + (pc2 - pbuf)
; if( uintputlen >= 33 ) { DIE }
;      if namelen == 0x20   and   pwdlen = 0x1fe
;      max is 0x21f
;      min is 0x100
;   TRY: namelen = 32, pwdlen = 0x101 - 0x20 = 225
...
; char* msg;
; if( test_username_and_password_valid( localpass ) ) {
...
4638: "Invalid Password Length: password too long"
...
46d2:  0f12           push	r15
46d4:  0312           push	#0x0
46d6:  b012 6446      call	#0x4664 <INT>   ; putch
46da:  2152           add	#0x4, sp

puts_tst:
46dc:  6f4b           mov.b	@r11, r15
46de:  4f93           tst.b	r15
46e0:  f623           jnz	#0x46ce <puts_strt>

46e2:  3012 0a00      push	#0xa
46e6:  0312           push	#0x0
46e8:  b012 6446      call	#0x4664 <INT>  ; putch
